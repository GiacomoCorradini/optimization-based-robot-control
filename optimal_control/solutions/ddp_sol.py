...
def backward_pass(solver, X_bar, U_bar, mu):
        ...
        for i in range(N-1, -1, -1):
            ...
            A[i,:,:] = solver.f_x(X_bar[-1,:], U_bar[-1,:])
            B[i,:,:] = solver.f_u(X_bar[-1,:], U_bar[-1,:])
                    
            # compute the gradient of the cost function at X=X_bar
            solver.l_x[i,:]    = solver.cost_running_x(i, X_bar[-1,:], U_bar[-1,:])
            solver.l_xx[i,:,:] = solver.cost_running_xx(i, X_bar[-1,:], U_bar[-1,:])
            solver.l_u[i,:]    = solver.cost_running_u(i, X_bar[-1,:], U_bar[-1,:])
            solver.l_uu[i,:,:] = solver.cost_running_uu(i, X_bar[-1,:], U_bar[-1,:])
            solver.l_xu[i,:,:] = solver.cost_running_xu(i, X_bar[-1,:], U_bar[-1,:])
            ...
            V_x[i,:]    = solver.Q_x[i,:] + solver.K[i,:,:].T @ (solver.Q_u[i,:] + solver.Q_uu[i,:,:] @ solver.w[i,:]) + solver.Q_xu[i,:,:] @ solver.w[i,:]
            V_xx[i,:]   = solver.Q_xx[i,:,:] + solver.K[i,:,:].T @ solver.Q_uu[i,:,:] @ solver.K[i,:,:] + solver.Q_xu[i,:,:] @ solver.K[i,:,:] + solver.K[i,:,:].T @ solver.Q_xu[i,:,:].T
